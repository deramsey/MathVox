...
var LatexSyntax = (() => {
  ...
  function countLeaves(expr) {
    ...
    if (dict) {
      const dictRecord = dict;
      const keys = Object.keys(dictRecord);
      return 1 + keys.length + keys.reduce((acc, x) => acc + countLeaves(dictRecord[x]), 0);
    }
    return 0;
  }
  ...
  function isSinglePowerDenominator(denom) {
  ...
  function getIndexStyle(_expr, _level) {
    return "subscript";
  }
  ...
  function makeSerializeHandler(entry, latexTrigger, idTrigger) {
    ...
    if (latex) {
      const prec2 = fields.precedence ?? 1e4;
      if (kind === "postfix")
      ...
      if (kind === "infix") {
        return (serializer, expr) => {
          const n = nops(expr);
          if (n === 0) return "";
          const prec3 = fields.precedence ?? 1e4;
          return joinLatex(
    ...
    const prec = fields.precedence ?? 1e4;
    if (kind === "postfix")
  ...
  function isValidEntry(entry, onError) {
    const fields = entryFields(entry);
    let subject = entry.name ?? fields.latexTrigger ?? fields.symbolTrigger ?? fields.openTrigger;
    if (!subject) {
      try {
    ...
    if (isInfixEntry(entry) || isPostfixEntry(entry) || isPrefixEntry(entry)) {
      if (Array.isArray(entry.latexTrigger) && (entry.latexTrigger[0] === "_" || entry.latexTrigger[0] === "^") || typeof entry.latexTrigger === "string" && (entry.latexTrigger.startsWith("^") || entry.latexTrigger.startsWith("_"))) {
        if (entry.precedence !== void 0 || fields.associativity !== void 0) {
          onError({
            severity: "warning",
            message: [
              "invalid-dictionary-entry",
      ...
      if (fields.associativity !== void 0) {
        onError({
          severity: "warning",
          message: [
            "invalid-dictionary-entry",
  ...
  var BigDecimal = class _BigDecimal {
    ...
    sqrtToward(direction) {
      ...
      const sig = this.significand;
      const exp = this.exponent;
      const decExp = exp + bigintDigits(sig) - 1;
      const halfExp = Math.floor(decExp / 2);
      let shift = prec + guard - halfExp;
      if (exp + 2 * shift < 0) shift = Math.ceil(-exp / 2) + 1;
  ...
  function roundMagnitudeToward(mant, inexact, exp, prec, up) {
  ...
  function cbrtSeed(C) {
    ...
    if (remainder === 1) fs *= 1.2599210498948732;
    if (remainder === 2) fs *= 1.5874010519681994;
  ...
  var _Parser = class __Parser {
     ...
     */
    matchBoundaryTokens(tokens) {
      if (this.matchAll(tokens)) return true;
      if (tokens.length === 2 && CLOSE_DELIMITER_PREFIX.has(tokens[0]) && this._tokens[this.index] === tokens[0] && this._tokens[this.index + 1] === ".") {
        this.index += 2;
        return true;
      }
      return false;
    ...
    matchBoundary() {
      ...
      const match = currentBoundary && this.matchBoundaryTokens(currentBoundary.tokens);
    ...
    sourceOffsets(startToken, endToken = this.index) {
    ...
    matchDelimiter(open, close) {
      ...
      if (Array.isArray(open)) {
        ...
        if (open.length === 1) {
          ...
          const closeToken2 = close[0];
          const closeBoundary3 = closePrefix ? hasBracedDelimiter ? [closePrefix, "<{>", closeToken2, "<}>"] : [closePrefix, closeToken2] : [closeToken2];
      ...
      const closeToken = CLOSE_DELIMITER[open] ?? close;
      const closeBoundary = closePrefix ? hasBracedDelimiter ? [closePrefix, "<{>", closeToken, "<}>"] : [closePrefix, closeToken] : [closeToken];
     ...
     */
    parseStringGroup(optional, rawTokens) {
     ...
     */
    parseStyledInfixOperator(lhs, until) {
      const start = this.index;
      ...
      for (const def of defs) {
        ...
        if (def.closeTokens.size > 0) {
          ...
          for (let i = start; i < tokens.length; i++) {
            ...
            if (hasPrefix && CLOSE_DELIMITER_PREFIX.has(tokens[i]) && tokens[i + 1] === ".") {
              found = true;
              break;
            }
          }
      ...
      if (lhs !== null) {
        ...
        while (!done && !this.atTerminator(until)) {
          ...
          if (result === null && until.minPrec <= INVISIBLE_OP_PRECEDENCE) {
            ...
            if (opDefs.length === 0 || opDefs.every(
              ([def]) => def.latexTrigger === "\\text" || def.latexTrigger === "\\keyword"
            )) {
    ...
    error(code, fromToken) {
      ...
      const fn = latex ? ["Error", msg, ["LatexString", { str: latex }]] : ["Error", msg];
      const isMissing = typeof code === "string" && code === "missing";
      const sourceOffsets = isMissing ? this.sourceOffsets(fromToken, fromToken) : this.sourceOffsets(fromToken, this.index);
      ...
      for (let i = 0; i < digits.length - 16; i++) {
        ...
        for (let j = 0; j < 17; j++) {
          ...
          if ((offset + cycle.repeat(times + 1)).startsWith(truncatedDigits)) {
            ...
            const repeatingDecimalPatterns = {
              vinculum: "\\overline{#}",
              parentheses: "(#)",
              dots: "\\overset{\\cdots}{#1}#2\\overset{\\cdots}{#3}",
              arc: "\\wideparen{#}"
            };
            let pattern = repeatingDecimalPatterns[options.repeatingDecimal] ?? "\\overline{#}";
  ...
  function serializeLatex(expr, dict, options) {
    const serializer = new Serializer6(dict, options);
