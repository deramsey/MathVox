...
class SemanticSkeleton {
    ...
    static fromStructure(mml, tree, options) {
        const idMap = new Map();
        if (mml.hasAttribute(enrich_attr_js_1.Attribute.ID)) {
            idMap.set(mml.getAttribute(enrich_attr_js_1.Attribute.ID), mml);
        }
        for (const elem of DomUtil.querySelectorAllByAttr(mml, enrich_attr_js_1.Attribute.ID)) {
            idMap.set(elem.getAttribute(enrich_attr_js_1.Attribute.ID), elem);
        }
        return new SemanticSkeleton(SemanticSkeleton.tree_(idMap, tree.root, options));
    }
    ...
    static tree_(idMap, node, options, level = 0, posinset = 1, setsize = 1) {
        if (!node) {
            return [];
        }
        const id = node.id;
        ...
        const children = SemanticSkeleton.combineContentChildren(node.type, node.role, node.contentNodes, node.childNodes);
        if (mmlChild) {
            SemanticSkeleton.addOwns_(mmlChild, children);
        }
        for (let i = 0, l = children.length, child; (child = children[i]); i++) {
            skeleton.push(SemanticSkeleton.tree_(idMap, child, options, level + 1, i + 1, l));
        }
        SemanticSkeleton.addAria(mmlChild, level, posinset, setsize, options);
        return skeleton;
    }
