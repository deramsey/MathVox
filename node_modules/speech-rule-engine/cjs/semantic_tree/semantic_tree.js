...
function checkDuplicateMathmlTrees(root) {
    const seen = new Map();
    const visit = (node) => {
        if (node.mathmlTree) {
            const nodes = seen.get(node.mathmlTree) || new Set();
            nodes.add(node);
    ...
    for (const [mathmlTree, nodes] of seen) {
        if (nodes.size > 1) {
            messages.push(`DUPLICATE mathmlTree: ${DomUtil.tagName(mathmlTree)} shared by ` +
                Array.from(nodes)
...
class SemanticTree {
    ...
    constructor(mathml, options) {
        ...
        debugger_js_1.Debugger.getInstance().generate(() => checkDuplicateMathmlTrees(this.root));
    }
    xml(opt_brief) {
